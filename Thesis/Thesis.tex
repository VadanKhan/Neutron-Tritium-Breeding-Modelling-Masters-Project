...
particle balance in phase space.
...
inversely with the square root of the number of simulated particles ($N$):
\begin{equation}
    R \propto \frac{1}{\sqrt{N}}
\end{equation}
To ensure statistical robustness, the simulations in this work utilized high particle counts
...
\subsubsection{OpenMC:}
    All simulations in this project are performed using \textbf{\texttt{OpenMC}}, a
    state-of-the-art, open-source Monte Carlo transport code \cite{romano_openmc_2015}.
    \texttt{OpenMC} is widely used in the fusion research community and has been extensively
    validated against other codes for complex fusion neutronics applications, demonstrating high
    accuracy in calculating parameters like TBR and volumetric heating \cite{bae_arc_2022}.

\subsubsection{Paramak:} To manage the complex reactor geometry, this project also uses
...
\begin{figure}[htbp]
    ...
    \caption{Figure indicating our simplified Tokamak Geometry built with paramak. In the
    computation we insetad use a 90° revolution for computational efficiency, with periodic boundary
    conditions on each radial slice to ensure physical accuracy.}
...
\begin{enumerate}
    ...
    Geometry Monte Carlo) unstructured mesh geometry, a format optimised for particle transport
    codes.
...
higher packing efficiency \cite{goel_optimization_2023,segantin_optimization_2020}. While high TBR
...
Table~\ref{tab:solid_breeders} indicates that solid ceramics display less performance variation than
liquids, as they share similar lithium-oxide bases. Compounds containing Beryllium multipliers
generally yield higher TBRs , while Cerium ceramics achieved the highest solid TBR due to low
...
Figure~\ref{fig:enrichment} illustrates the sensitivity of TBR to $^6$Li enrichment. As expected,
increasing enrichment yields a higher TBR, although the rate of increase diminishes at higher
percentages. Based on these results, an enrichment of 60\% was selected for subsequent simulations,
balancing performance with the diminishing returns observed in the plateau region.
...
\begin{figure}[htbp]
    ...
    curve exhibits diminishing returns but maintains a positive gradient even at high enrichment
    levels, indicating no hard saturation point within the tested range.}
    \label{fig:enrichment}
\end{figure}

The optimisation of the lead multiplier fraction in \ce{LiPb} breeders is shown in
Figure~\ref{fig:multiplier}. The TBR contribution peaks at a lead concentration of approximately
50\%, before dropping sharply at concentrations exceeding 75\%.

\begin{figure}[htbp]
    \centering
    % You can adjust the width as needed for your document layout
    \includegraphics[width=0.7\textwidth]{figures/LL New Chi.png}
    \caption{TBR dependence on Lead fraction in the $\text{Li}_{(100-x)}\text{Pb}_x$ breeder.
    Breeding performance peaks at approximately 50\% lead concentration, with a rapid decline
    observed beyond 75\%. The eutectic point is marked for reference. Error bars are included but
...
fraction increases. This behavior indicates that these gases effectively act as voids within the
...
declining. This is attributed to the moderating properties of water, which thermalizes the neutron
...
breeder volume at lower coolant fractions.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Impact of coolant choice on breeding performance. (a) TBR vs. Coolant Volume Fraction.
    While gaseous coolants (\ce{He}, \ce{N}) act as voids, \ce{H_2O} acts as a moderator, mitigating
    the TBR drop-off at high coolant fractions. Error bars are included but are too small to be
    visible at this scale.(b) Incident Neutron Energy spectrum. The \ce{H_2O} spectrum shows
    significant thermalization compared to the gaseous coolants, increasing the interaction
    cross-section. }
    \label{fig:coolants}
\end{figure}
...
parasitic absorption in the structural material. An average thickness of 25 mm was chosen for
simulations based on literature standards. Material comparison reveals that while EUROFER is the
standard structural choice, Zirconium-4 alloy provides superior neutronic performance.
...
\begin{figure}[htbp]
    ...
    materials. Zirconium-based alloys demonstrate the lowest parasitic absorption, maintaining the
    highest TBR profile. Error bars are included but are too small to be visible at this scale.}

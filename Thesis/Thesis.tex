...
\begin{itemize}
    ...
    a portion of its kinetic energy to the nucleus. In \textbf{elastic scattering $\ce{(n,n^{\prime})}$}, kinetic
    energy is conserved. In \textbf{inelastic scattering $\ce{(n,n^{\prime})}$}, the neutron excites the nucleus,
    ...
    capture \ce{(n, \gamma)}} event, where the nucleus emits a gamma ray. This is often a parasitic
    reaction, as it removes a neutron from the system that could have been used for breeding.
    Alternatively, the absorption can induce \textbf{charged particle emission}, such as \ce{(n,p)} or
    \ce{(n, \alpha)} reactions. This is the fundamental mechanism used for tritium breeding
...
tritium-producing reactions in lithium. The two key breeding reactions are mentioned in

[MISSING REFERENCE TO EQUATIONS]
...
While natural lithium itself has a minor neutron-multiplying reaction via the high-energy
$\ce{(n,n^{\prime}t)}$ reaction with \ce{^{7}Li}:
$$\ce{^{7}Li + n -> \alpha + t + n^{\prime}} - 2.5 MeV$$ This reaction is endothermic, consuming 2.5
MeV of energy, and its cross-section is insufficient on its own to guarantee tritium
self-sufficiency \cite{hernandez_first_2018}. Therefore, dedicated multiplier materials are
required, with the two primary candidates being Beryllium (Be) and Lead (Pb).

\begin{itemize}
    \item \textbf{Beryllium (Be):} Beryllium, specifically the isotope \ce{^{9}Be}, is an extremely
    effective neutron multiplier with a low energy threshold for its \ce{(n, 2n)} reaction:
    $$\ce{^{9}Be + n -> ^{8}Be + 2n -> 2\alpha + 2n}$$ It is widely used in conceptual
    solid breeder designs, such as the Helium-Cooled Pebble Bed (HCPB) and Water-Cooled Pebble Bed
    (WCPB), often in the form of beryllium pebbles or beryllide ceramics (\ce{Be12Ti})
    \cite{wang_current_2019, liu_design_2022}. It is also a key component in the molten salt FLiBe
    \ce{(2LiF . BeF2)} \cite{tas_liquid_1988, sorbom_arc_2015}. However, beryllium has two major
    ...
    \item \textbf{Lead (Pb):} Lead, typically natural lead or lead enriched in \ce{^{208}Pb}, is the
    other main multiplier candidate. It undergoes a high-energy \ce{(n, 2n)} reaction:
    $$\ce{^{208}Pb + n -> ^{207m}Pb + 2n}$$ Lead's primary advantage is that it can be
    seamlessly integrated into a liquid breeder, forming the \ce{Li17Pb83} eutectic alloy. This
    ...
    effective for breeding with \ce{^{6}Li}. Furthermore, its activation by high-energy neutrons can
...
\begin{itemize}
    ...
    the eutectic alloy \ce{Li17Pb83} \cite{mas_de_les_valls_leadlithium_2008, wu_overview_2011}.
    ...
    The most prominent example is \textbf{FLiBe \ce{(2LiF . BeF2)}}, which advantageously combines the
    breeder (LiF) and multiplier (\ce{BeF2}) into a single, low-conductivity fluid
    ...
    segantin_optimization_2020}. A wide variety of other salt compositions, including \ce{LiF.PbF2}
...
\begin{itemize}
    ...
    Li-Pb, \ce{Li4SiO4}, \ce{Li2TiO3}) chosen not only for breeding but for their structural,
    ...
    crucially, by systematically varying key parameters such as \textbf{\ce{^{6}Li} enrichment},

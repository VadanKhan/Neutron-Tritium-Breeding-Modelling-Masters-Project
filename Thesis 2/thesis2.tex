...
\begin{titlepage}
    ...
    optimised TBR was $1.41\pm0.003$. The bespoke SOBO-GPR pipeline proved highly effective in navigating this
...
spallation events in the structural materials (Section \ref{sec:methodology_summary}), notably
within the zirconium plamsa-facing First Wall (FW) ($\ce{^{90}Zr(n, 2n)^{89}Zr}$, $Q = -12.0
\text{ MeV}$, $\sigma \approx 0.6 \text{ b}$) and the EUROFER97  steel separators ($\ce{^{56}Fe(n,
...
within the FW via an extreme threshold capture reaction ( $\ce{^{90}Zr(n,t)^{88}Y}$, $Q = -11.3
\text{ MeV}$, $\sigma \sim 1 \text{ \textmu b}$). Conversely, highly thermalised neutrons within the
coolant layers can undergo radiative capture with the naturally occurring deuterium in water to
breed trace amounts of tritium ($\ce{^{2}H(n,\gamma)^{3}H}$, $Q = +6.26 \text{ MeV}$, thermal
$\sigma \approx 0.5 \text{ mb}$).
...
to those that have a long path in the cell, to give a more physically representative tally to be
tracked in neutron simulations.

\subsubsection{Radiation Damage and Displacements Per Atom COULD REDUCE}
\label{sec:theory_DPA}
To accurately assess the structural viability of the breeder blanket and FW, the neutronic
...
The surrogate model acts as a computationally inexpensive interpolator and extrapolator for the
OpenMC physics solver. This project utilises Gaussian Process Regression (GPR)
\citep{rasmussen2006gaussian}. Unlike parametric regression models that fit a single discrete curve,
a Gaussian Process (GP) defines a non-parametric probability distribution over all possible
functions that could fit the observed data.
...
that transforms the GP's $\mu(x)$ and variance into a single scalar `utility' score. The parameter
...
aggressively explore highly uncertain regions (Appendix \ref{app:acquisition_function}).
...
extraction. extraction. The FW endures the most severe environmental stressors in the reactor;
demonstration power plant (DEMO) concepts must mitigate surface heat fluxes often exceeding
...
for this architecture are explicitly fixed: the FW utilises a zirconium alloy, the breeder
...
$500\text{ MW}$ was assumed to scale DPA/FPY (Section \ref{sec:theory_DPA}), reflecting
the planned operational output of ITER \citep{aymar_iter_2002}.
...
mesh defined across the reactor. Within this grid, six specific reaction metrics were scored
independently using OpenMC's standard tallies: neutron flux, scattering density, tritium production,
neutron multiplication, nuclear heating, and damage energy. The raw outputs from these mesh tallies
were subsequently processed into a 2D poloidal slice of the reactor and visualised as heat maps
...
exploration-exploitation trade-off via the hyperparameter $\beta$. Properly tuning this parameter is
vital for a high-dimensional physical system where multiple local maxima may exist. Figure
\ref{fig:ucb_tuning} illustrates a 2D projection of the acquisition utility landscape used to
benchmark the model's behaviour.
...
with this highly explorative $\beta = 1,000,000$ value to ensure robust, global mapping of the
design space before narrowing in on the optimal layer thicknesses.
...
parameter, could occasionally stagnate in discovered maxima (Figure \ref{fig:2d_gramacy_lee}). To
...
In successfully navigating notoriously difficult mathematical landscapes and recreating them
...
\begin{figure}[htbp]
    ...
    landscapes. (a) A 1D Schwefel test case illustrating full surrogate model reconstruction (model
    uncertainty is plotted but too small to be visible). (b) 2D Gramacy \& Lee function test
    illustrating global trend reconstruction but localised peak ignorance. (c) 2D Hidden Peak
    function test successfully escaping and finding the true peak. (d) 2D multi-modal
    Schwefel function test demonstrating complete reconstruction of the detailed space. Pink markers
    annotate where a secondary ``peak hunt'' is triggered. Trials ($N$): (a, b, d) used $N=150$,
    while (c) used $N=100$. All tests utilised $N_{sobol}=50$ and UCB $\beta=1,000,000$. Bespoke
    algorithm enhancements were not enabled for (a) and (b). Enhancements in (c) and (d) used
    $\epsilon = 0.75$, and $n_{hunt}=10$.}
...
functions was easy with a tuned base SOBO-GPR algorithm. However, Figure \ref{fig:2d_hidden_peak}
and \ref{fig:2d_schwefel} demonstrate the utility of the bespoke enhancements. The algorithm was
able to handle the known deceptively difficult hidden peak problem (where a narrower, slightly
taller peak is hidden among a wide, clear peak) with confidence, utilising the duplicate avoidance
and multi-modal exploitation algorithms to avoid getting trapped. Trialling it further with more
smaller peaks in the 2D schwefel indicates that the increased exploration allows for not just a
successful optimisation, but a well explored and accurate final surrogate.
...
overall trends, but exhibited less sensitivity to small-length-scale variations. Note that while the
algorithm was designed to be `over-prepared' to handle small localised peaks, there remains a risk
of optimiser local peak entrapment, yielding a marginally suboptimal result.
...
within the breeder material, weak absorption in the FW, slightly stronger interactions in
...
by the FW increases in forward-weighted configurations ($\zeta < 0$). This coincides with a
corresponding increase in total damage energy deposited into the FW. The spatial
...
eutectic alloy would be liquid, we focus on solid separators and the FW. In rear-weighted
configurations, the internal structural separators are exposed to high-energy source neutrons much
earlier, driving a stark increase in internal DPA. Conversely, the FW experiences peak DPA
...
stark increase in internal DPA. Conversely, the FW experiences peak DPA in forward-weighted
...
\citep{woods_properties_1966}) Zircaloy-4 restricts the FW lifetime to roughly $8$--$10$
FPYs—far below ITER's 30-year operational target \citep{aymar_iter_2002}. By contrast,
an ultra-fine grained EUROFER97 FW ($2.2$--$2.5$ DPA/year, $57.47$ threshold DPA
...
such as neutron streaming. To navigate this design space, the bespoke SOBO-GPR pipeline proved
highly effective, identifying that optimal TBR demands a front-loaded (lower $\zeta$), `binding
